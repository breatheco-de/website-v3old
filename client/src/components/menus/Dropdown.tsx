...
export function Dropdown({ label, href, dropdown, controlledOpen, onOpenChange, subtleAtTop }: DropdownProps & { controlledOpen?: boolean; onOpenChange?: (open: boolean) => void }) {
  ...
  return (
    ...
    >
      <button
        type="button"
        className={`flex items-center gap-1 px-4 py-2 font-medium text-foreground hover-elevate rounded-md transition-[font-size] duration-300 no-default-hover-elevate no-default-active-elevate ${subtleAtTop ? "text-xs" : "text-sm"}`}
