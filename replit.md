...
-   **ContentIndex Singleton**: `server/content-index.ts` is the single source of truth for all YAML content read/write operations. It provides `safeYamlLoad` (template-variable-safe YAML parsing), `loadLocaleData` (locale file only, no `_common.yml` merge — use for writes), `loadMergedContent` (merged with `_common.yml` — use for validation/display), `loadCommonData`, `loadContent` (schema-validated merged loading), `getContentFilePath`, `getContentFolderPath`, `getCommonFilePath`, `listContentSlugs`, and `getAvailableLocalesOrVariants`. All content file read/write operations in `content-editor.ts`, `bindings.ts`, `sitemap.ts`, and `routes.ts` use these methods to avoid duplicating `_common.yml` merge logic, ensure safe template variable handling, and prevent accidental content duplication in locale files. NEVER use raw `yaml.load()` for content files — always use `contentIndex.safeYamlLoad()` or the higher-level methods.
