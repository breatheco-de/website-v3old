...
-   **Blog System**: Legacy blog integration via Breathecode API. Configuration in `marketing-content/blog.yml` defines API endpoint, auth token env var name (`BREATHECODE_TOKEN`), cache TTL (default 24h), and URL patterns. Backend service (`server/blog.ts`) fetches posts, caches the JSON response locally (`.cache/blog-posts.json`), and auto-expires based on TTL. Blog API routes: `GET /api/blog/posts?locale=`, `GET /api/blog/posts/:slug`, `GET /api/blog/cache-status`, `POST /api/debug/clear-blog-cache`. Frontend pages: `BlogListingPage` at `/en/blog` and `/es/blog`, `BlogPostPage` at `/en/blog/:slug` and `/es/blog/:slug`. SSR meta tags and Schema.org `BlogPosting` structured data injected server-side. Blog cache clearable via debug bubble. Blog posts included in sitemap.
