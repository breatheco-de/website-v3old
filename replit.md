...
-   **GitHub Content Sync**: Allows content edits made via the inline editor to be committed back to a GitHub repository. Includes an **Auto-Commit System** (`server/auto-commit.ts`) that automatically commits marketing-content file changes to GitHub on a configurable throttled schedule (default 5s). Uses Tree API for batch commits with Contents API fallback for conflict resolution. Sync state is persisted to GCS bucket (`sync/sync-state.json`) to survive deployments. API endpoints at `/api/github/auto-commit/*` for status, flush, config, and conflict management.
