...
-   **Content Loader Utilities**: Centralized YAML file I/O in `server/utils/contentLoader.ts`. Provides `loadLocaleData` (locale file only, no `_common.yml` merge — use for writes), `loadMergedContent` (merged with `_common.yml` — use for validation/display), `loadCommonData`, `getContentFilePath`, and `getContentFolderPath`. All content file read/write operations in `content-editor.ts` and `bindings.ts` use these helpers to avoid duplicating `_common.yml` merge logic and prevent accidental content duplication in locale files.
